...
use isvalina_20\Steta;
use isvalina_20\Pomocno;

Flight::route('GET /v1/prijave', function(){
   $doctrineBootstrap = Flight::db();
   $em = $doctrineBootstrap->getEntityManager();
   $repozitorij=$em->getRepository('isvalina_20\Steta');
...
Flight::route('POST /v1/prijava', function(){
  ...
  if(strlen(Flight::request()->data->geografskaSirina)===0){
    Flight::json(Pomocno::odgovor(true,'Geografska sirina obavezno',null));
    return;
  }
  if(strlen(Flight::request()->data->geografskaDuzina)===0){
    Flight::json(Pomocno::odgovor(true,'Geografska duzina obavezno',null));
    return;
  }
  if(strlen(Flight::request()->data->iznosStete)===0){
    Flight::json(Pomocno::odgovor(true,'Procijenjeni iznos stete obavezno',null));
    return;
  }
